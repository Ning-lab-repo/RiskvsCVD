...
# 文件路径
input_paths = [
    r"G:\CVD\3.KNN\DM\DM1+不稳定心绞痛.xlsx",
    r"G:\CVD\3.KNN\DM\DM2+急性心肌梗死.xlsx",
    r"G:\CVD\3.KNN\DM\DM3+慢性缺血性心脏病.xlsx",
    r"G:\CVD\3.KNN\DM\DM4+脑梗死.xlsx",
    r"G:\CVD\3.KNN\DM\DM5+脑内出血.xlsx",
    r"G:\CVD\3.KNN\DM\DM6+脑血管病后遗症.xlsx",
    r"G:\CVD\3.KNN\DM\2DM7+总CVD.xlsx"
]
...
for input_path in input_paths:
    ...
    **xgb_params, 
    ...
    # 评估模型在测试集上的性能
    y_pred_rf_proba = xgb.predict_proba(X_test)[:, 1]  # 获取正类的概率
    ...
    # 绘制 ROC 曲线
    plt.plot(fpr_rf, tpr_rf, label=f"DM vs {label} (AUC = {roc_auc_rf:.4f})")
...
plt.title("Diabetes mellitus", fontsize=25)
...
# 保存图像为PNG
save_path_png = r"G:\CVD\5.ROC\6.DM\2.DM.png"
plt.savefig(save_path_png, dpi=300, bbox_inches='tight')  # 确保保存图像时不裁剪内容
print(f"ROC 曲线已保存至: {save_path_png}")

# 保存图像为PDF
save_path_pdf = r"G:\CVD\5.ROC\6.DM\2.DM.pdf"
plt.savefig(save_path_pdf, bbox_inches='tight')  # 不需要dpi，PDF是矢量格式
print(f"ROC 曲线已保存至: {save_path_pdf}")
